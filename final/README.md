...
AI endpoint가 죽어 있거나 ngrok upstream이 연결되지 않으면 전체 변환은 중단하지 않는다. 파서 기반 `questions_final.json`을 저장하고, 실패 정보는 `metadata.text_refinement` 또는 `metadata.ai_enrichment`에 기록한다. JSON 파싱 실패처럼 같은 원인으로 반복될 가능성이 높은 실패가 `--ai-max-failures`에 도달하면 남은 AI 작업은 건너뛴다. 단, timeout은 일시 지연으로 보고 `metadata.text_refinement.timeout_questions`에 기록하되 skip 기준에는 포함하지 않는다.
