...
def load_shared_batch_module():
    module_path = Path(__file__).resolve().with_name("_batch_image_groups.py")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        help="결과 txt 저장 디렉토리. 생략하면 input_dir 상위에 exam_answer_txt를 사용",
    )
    parser.add_argument("--test-id", type=int, default=1, help="기본값은 1")
    parser.add_argument(
...
def get_runner_script_path() -> Path:
    return Path(__file__).resolve().with_name("generate_answer.py")


def build_output_dir(*, input_dir: Path, output_dir: str | None) -> Path:
    if output_dir:
        return Path(output_dir).expanduser().resolve()
    return input_dir.parent / "exam_answer_txt"


def build_output_path(*, output_dir: Path, question_number: int) -> Path:
    return output_dir / f"question_{question_number:03d}_answer.txt"
