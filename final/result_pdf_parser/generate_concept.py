...
PROMPT_TEMPLATE = """
너는 시험 문제 이미지들을 분석하여, 여러 문제가 공통으로 설명하거나 평가하는 핵심 학습 개념 1개를 추출하고, 이를 학생용 학습 개념 JSON으로 정리하는 도우미다.

입력으로는 문제 이미지가 1장 이상 주어진다.
각 문제의 문제문과 선택지를 읽고, 모든 문제를 함께 포괄하는 공통 상위 개념 1개를 추론하라.

[목표]
- 각 문제의 정답 자체보다, 여러 문제가 공통으로 다루는 핵심 개념 1개를 찾는다.
- 출력 concept는 반드시 정확히 1개만 생성한다.
- 문제마다 세부 포인트가 달라도, 이를 묶는 더 상위의 학습 개념으로 일반화한다.
- 어떤 문제는 상위 개념, 어떤 문제는 그 하위 개념을 묻더라도, 둘을 포괄하는 하나의 상위 개념으로 통합한다.
...
- 문제들 사이에 직접적인 공통 세부 항목이 없더라도, 같은 단원이나 학습 주제를 포괄하는 상위 개념이 있으면 그것을 선택한다.
...
- 들여쓰기는 스페이스 2칸으로 한다.
- concepts 배열에는 항목을 정확히 1개만 넣는다.
- question_concept_mapping에는 입력된 모든 question_id를 각각 정확히 한 번씩만 넣는다.
- 모든 mapping의 concept_id는 유일한 그 concept_id 하나를 가리켜야 한다.
...
1. JSON 외의 텍스트가 없는가?
2. concepts 배열 길이가 정확히 1인가?
3. 모든 question_id가 question_concept_mapping에 정확히 한 번씩만 들어갔는가?
4. 모든 mapping이 동일한 concept_id 하나만 가리키는가?
5. 모든 key가 ASCII double quote(")로 감싸져 있는가?
6. 모든 문자열 값이 ASCII double quote(")로 감싸져 있는가?
7. “ ” ‘ ’ ＂ 문자가 전혀 없는가?
8. 출력이 스페이스 2칸 들여쓰기의 pretty JSON인가?
9. trailing comma가 없는가?
10. explanation이 충분히 자세한 학생용 설명인가?
11. 문제별 정답 해설이 아니라 공통 개념 중심 설명인가?
12. 최종 결과가 유효한 JSON인가?
...
def load_shared_module():
    module_path = Path(__file__).resolve().with_name("generate_answer.py")
...
def build_text_format() -> dict:
    return {
        "format": {
            "type": "json_schema",
            "name": "exam_concepts_json_single_common_concept",
            "strict": True,
...
def validate_response_payload(payload: dict, expected_question_ids: list[int]) -> None:
    ...
    if not isinstance(concepts, list) or len(concepts) != 1:
        raise ValueError("concepts는 정확히 1개여야 한다.")
    ...
    expected_ids = sorted(expected_question_ids)
    mapped_question_ids = []
    for mapping in mappings:
        ...
        if mapping["concept_id"] != concept["concept_id"]:
            raise ValueError("모든 mapping은 유일한 concept_id 하나만 가리켜야 한다.")
        mapped_question_ids.append(mapping["question_id"])

    if sorted(mapped_question_ids) != expected_ids:
        raise ValueError("모든 question_id가 정확히 한 번씩 매핑되어야 한다.")
