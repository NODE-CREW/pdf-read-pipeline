...
"""
Usage:
  python ./5_extract_all_text_and_save_latex_split_images.py --pdf ./level2.pdf
...
@lru_cache(maxsize=1)
def _load_split_passage_and_choices() -> Callable[[str], Tuple[str, str]]:
    module_path = Path(__file__).resolve().parent / "1_extract_text_and_print.py"
    spec = importlib.util.spec_from_file_location("extract_text_and_print", str(module_path))
    if spec is None or spec.loader is None:
        raise RuntimeError("split_passage_and_choices 로더를 준비하지 못했습니다.")
    module = importlib.util.module_from_spec(spec)
    sys.modules["extract_text_and_print"] = module
    spec.loader.exec_module(module)

    split_fn = getattr(module, "split_passage_and_choices", None)
    if split_fn is None:
        raise RuntimeError("1_extract_text_and_print.py 에 split_passage_and_choices 가 없습니다.")
    return split_fn


def split_question_and_choices(question_text: str) -> Tuple[str, str]:
    split_fn = _load_split_passage_and_choices()
    return split_fn(question_text)
