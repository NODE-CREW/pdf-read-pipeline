...
def load_module():
    module_name = "generate_exam_concepts_json_to_txt"
    module_path = Path(__file__).resolve().parents[1] / "9-1_generate_exam_concepts_json_to_txt.py"
...
def test_parse_and_validate_response_text_rejects_missing_question_mapping():
    ...
    with pytest.raises(ValueError, match="모든 question_id"):
...
def test_request_retries_when_response_contains_smart_quotes(tmp_path):
    module = load_module()
    image1 = tmp_path / "q1.png"
    image2 = tmp_path / "q2.png"
    write_minimal_png(image1)
    ...
    responses = iter(
        [
            '{"concepts":[{"concept_id":200,"concept_name":"공통 개념","summary":"요약이다. 요약이다.","explanation":"문단1\\n\\n문단2\\n\\n문단3\\n\\n“문단4”"}],"question_concept_mapping":[{"mapping_id":1,"question_id":7,"concept_id":200},{"mapping_id":2,"question_id":8,"concept_id":200}]}',
