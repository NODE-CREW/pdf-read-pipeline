...
def load_module():
    module_name = "extract6_1_db_ready_module"
    module_path = (
        Path(__file__).resolve().parents[1] / "6-1_extract_all_text_and_save_latex_split_images.py"
    )
...
def _build_fixture(module6, module5):
    question_images = [
...
def test_build_db_ready_records_includes_shared_fields_and_relative_paths():
    ...
    question_images, question_texts, shared_passages, shared_map = _build_fixture(module6, module5)

    records = module6.build_db_ready_records(
...
def test_build_db_ready_records_record_id_is_stable_for_same_input():
    ...
    records_a = module6.build_db_ready_records(
        pdf_path="/tmp/input/level4.pdf",
        target_dir=Path("/tmp/output/level4"),
        question_images=question_images,
        question_texts=question_texts,
    ...
    records_b = module6.build_db_ready_records(
        pdf_path="/tmp/input/level4.pdf",
        target_dir=Path("/tmp/output/level4"),
        question_images=question_images,
        question_texts=question_texts,
...
def test_save_db_ready_jsonl_writes_one_line_per_question(tmp_path):
    ...
    out_path = module6.save_db_ready_jsonl(tmp_path, records)
...
def test_save_split_texts_and_db_ready_jsonl_can_coexist(tmp_path):
    ...
    out_path = module6.save_db_ready_jsonl(out_dir, records)
