...
`test-1.pdf` 전용 구현이라 페이지별 상/하단 경계를 고정값으로 사용합니다.
...
- **2~7페이지**: 일반 페이지이므로 `y < 60`, `y > 790`를 버립니다.
- **8페이지**: 문제 본문이 아니라 정답표 페이지라 본문 추출 대상에서 제외됩니다.
...
2. 텍스트로 충분히 표현 가능한 후보는 버리고, 놓친 트리 도형은 bbox 렌더 후 텍스트 마스킹 방식으로 fallback 복구
...
이후 각 후보에 대해 아래 필터를 적용합니다.

- **텍스트-heavy 후보 제거**: 후보 rect 안의 텍스트 점유율이 높으면 버림
- **텍스트로 표현 가능한 코드/SQL/숫자리스트 제거**: 이미 `question_text`에 충분히 들어간 경우 crop 생략
...
8페이지 정답표는 현재 문제 추출에서 제외할 뿐이고, 별도의 `answer` 필드로 매핑하지는 않습니다.
...
> **OCR이 아니라 `fitz`의 텍스트 좌표를 읽고, 2단 컬럼을 강제로 복원한 뒤, 문제 번호/선택지 패턴으로 구조화하고, 마지막에 표/트리 같은 비텍스트 정보만 별도로 crop 하는 방식**

그래서 일반 추출기로는 깨지던 문항 순서를 복원할 수 있었고, `@output` 계열 JSON과 함께 **트리/표 중심의 시각 정보 crop**까지 만들 수 있습니다. 현재 구현은 범용 파서가 아니라, **`test-1.pdf` 검수 통과를 목표로 튜닝된 전용 파서**로 이해하는 것이 가장 정확합니다.
