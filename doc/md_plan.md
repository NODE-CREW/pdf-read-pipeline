...
- [x] **2.1** JSON 요소에서 이미지 대상 필터링 함수 작성
  - 입력: JSON 요소 리스트
  - 출력: `table`, `picture`, `formula` 타입 요소 리스트
- [x] **2.2** bounding box 기반 이미지 crop 함수 작성
...
- [x] **3.1** JSON 요소 → 마크다운 변환 함수 작성
  - `heading` → `# 제목`
  - `paragraph` → 텍스트
  - `list` → 마크다운 리스트
- [x] **3.2** 이미지 요소 → 마크다운 이미지 참조 삽입
  - `![alt](./images/xxx.png)`
- [x] **3.3** 요소 순서 정렬 (reading order 기반)
- [x] **3.4** 마크다운 문서 조립 및 파일 저장
- [ ] **3.5** 단위 테스트 작성

### Phase 4: 시험지 특화 처리

- [x] **4.1** 기존 문항 경계 추정 로직 통합
  - `pipelines/base.py` 재사용
- [x] **4.2** 문항별 마크다운 분리 저장
